  ...
  <div class="lists">
    <v-tabs v-model="tabs" color="transparent" slider-color="#8e56d9" class>
      <v-tab href="#raids" class="primary--text">
        Raids
        <div v-if="futureRaids.length+activeRaids.length > 0" class="items__counter">
          <span>{{futureRaids.length+activeRaids.length}}</span>
        </div>
      </v-tab>
      <v-tab href="#quetes" class="primary--text">
        Quêtes
        <div v-if="activeQuests.length > 0" class="items__counter">
          <span>{{activeQuests.length}}</span>
        </div>
      </v-tab>
      <v-tab href="#invasions" class="primary--text">
        Rocket
        <div v-if="activeRocketInvasions.length > 0" class="items__counter">
          <span>{{activeRocketInvasions.length}}</span>
        </div>
      </v-tab>
    </v-tabs>
    ...
    <v-tabs-items v-model="tabs">
      <v-tab-item value="raids">
        <div
          v-if="futureRaids.length > 0 || activeRaids.length > 0"
          style="flex-basis: 100%; padding-bottom: 70px"
        >
              ...
              <div v-on:click="showModal(gym)" v-for="gym in activeRaids" class="raid__wrapper">
                ...
                <div class="raid__content">
                  <h3>
                    <span
                      v-if="gym.raid.ex"
                    >Raid EX de {{getRaidStartTime(gym.raid)}} à {{getRaidEndTime(gym.raid)}}</span>
                    <span
                      v-if="gym.raid.egg_level === 7"
                    >Méga-raid de {{getRaidStartTime(gym.raid)}} à {{getRaidEndTime(gym.raid)}}</span>
                    <span
                      v-else
                    >{{gym.raid.egg_level}}T de {{getRaidStartTime(gym.raid)}} à {{getRaidEndTime(gym.raid)}}</span>
      ...
      <v-tab-item value="invasions">
        <div
          v-if="activeRocketInvasions.length > 0"
          style="flex-basis: 100%; padding-bottom: 70px;"
        >
          <div class="raids__active">
            ...
            <div class="raids__wrapper">
              <div
                ...
                v-for="gym in activeRocketInvasions"
                class="raid__wrapper"
              >
                <div class="raid__img">
    ...
    <v-dialog v-model="dialog" max-width="290" content-class="list-filters">
      <v-card v-if="tabs == 'raids'">
        ...
        <v-card-text>
          ...
          <v-checkbox v-model="raidsListFilters" label="Méga-raids" value="7"></v-checkbox>
...
export default {
  ...
  computed: {
    ...
    activeRaids() {
      ...
      return this.$store.getters.activeRaids
        ...
        .filter(function (gym) {
          ...
          let inArray = that.raidsListFilters.includes(
            gym.raid.egg_level.toString()
          )
            ? true
            : false;
          let isEmptyZone = that.raidsZoneFilters.length == 0 ? true : false;
          let inZone =
            gym.zone && that.raidsZoneFilters.includes(gym.zone.id)
              ? true
              : false;
    ...
    activeQuests() {
      ...
      return this.$store.getters.activeQuests.filter(function (gym) {
        ...
        let inArray =
          gym.quest.reward &&
          that.questsListFilters.filter(
            (reward) => reward.name == gym.quest.reward.name
          ).length > 0
            ? true
            : false;
        let isEmptyZone = that.questsZoneFilters.length == 0 ? true : false;
        let inZone =
          gym.zone && that.questsZoneFilters.includes(gym.zone.id)
            ? true
            : false;
        return (isEmpty || inArray) && (isEmptyZone || inZone);
      });
  ...
  methods: {
    ...
    addReward(selectedOption, id) {
      ...
      if (
        this.questsListFilters.length > 0 &&
        this.questsListFilters.filter(
          (reward) => reward.name == selectedOption.name
        ).length > 0
      )
        return;
