            ...
            <v-card flat>
              <v-card-text>
                  ...
                  <div v-for="step in steps" class="settings-section">
                      ...
                      <div v-for="(pokemon, index) in boss.pokemon['step'+step.id]" class="setting pokemon">
                          <img :src="pokemon.thumbnail_url">
                          <p>{{pokemon.name_fr}}</p>
                          <v-btn flat icon color="deep-orange" @click="removePokemonFromStep(index, step.id)">
                              <v-icon>close</v-icon>
                          </v-btn>
                      </div>
                  </div>
...
export default {
  ...
  addPokemonToStep(selectedOption, stepId) {
      if( this.boss.pokemon['step'+stepId].filter( pokemon => pokemon.id == selectedOption.id ).length > 0 ) return;
  ...
  save( args ) {
      ...
      this.loading = true;
      axios.put('/api/rocket/bosses/'+this.boss.id, args).then( res => {
