<template>
  <div class="mb-60">
    ...
    <div class="settings-section">
      <v-subheader>Boss de Mega-raid</v-subheader>
      <multiselect
        :reset-after="true"
        v-model="value"
        :options="pokemons"
...
export default {
  ...
  methods: {
    initBosses() {
      this.bosses1t = this.$store.state.pokemons.filter(
        (boss) => boss.boss_level == "1"
      );
      this.bosses2t = this.$store.state.pokemons.filter(
        (boss) => boss.boss_level == "2"
      );
      this.bosses3t = this.$store.state.pokemons.filter(
        (boss) => boss.boss_level == "3"
      );
      this.bosses4t = this.$store.state.pokemons.filter(
        (boss) => boss.boss_level == "4"
      );
      this.bosses5t = this.$store.state.pokemons.filter(
        (boss) => boss.boss_level == "5"
      );
      this.bosses6t = this.$store.state.pokemons.filter(
        (boss) => boss.boss_level == "6"
      );
      this.bosses7t = this.$store.state.pokemons.filter(
        (boss) => boss.boss_level == "7"
      );
    },
    addBossTo1t(selectedOption, id) {
      if (
        this.bosses1t.filter((boss) => boss.id == selectedOption.id).length > 0
      )
        return;
    ...
    addBossTo2t(selectedOption, id) {
      if (
        this.bosses2t.filter((boss) => boss.id == selectedOption.id).length > 0
      )
        return;
    ...
    addBossTo3t(selectedOption, id) {
      if (
        this.bosses3t.filter((boss) => boss.id == selectedOption.id).length > 0
      )
        return;
    ...
    addBossTo4t(selectedOption, id) {
      if (
        this.bosses4t.filter((boss) => boss.id == selectedOption.id).length > 0
      )
        return;
    ...
    addBossTo5t(selectedOption, id) {
      if (
        this.bosses5t.filter((boss) => boss.id == selectedOption.id).length > 0
      )
        return;
    ...
    addBossTo6t(selectedOption, id) {
      if (
        this.bosses6t.filter((boss) => boss.id == selectedOption.id).length > 0
      )
        return;
    ...
    addBossTo7t(selectedOption, id) {
      if (
        this.bosses7t.filter((boss) => boss.id == selectedOption.id).length > 0
      )
        return;
      this.bosses7t.push(selectedOption);
    },
    remove7t(index) {
      this.bosses7t.splice(index, 1);
    },
    submit() {
      const args = {
