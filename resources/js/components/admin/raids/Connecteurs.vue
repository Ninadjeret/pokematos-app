    ...
    <div>
        <div class="settings-section">
            ...
            <template v-for="(item, index) in filteredItems">
              <v-list-tile :key="item.id" :to="{ name: 'admin.raids.annonces.edit', params: { id: $route.params.id, connector_id:item.id } }">
                <v-list-tile-content>
                  <v-list-tile-title>
                      {{item.name}}
                  </v-list-tile-title>
            ...
            <v-btn dark fixed bottom right fab :to="{ name: 'admin.raids.annonces.add', params: { id: $route.params.id } }"><v-icon>add</v-icon></v-btn>
        </div>
    </div>
</template>
