  ...
  <div>
    <div class="settings-section">
      ...
      <div class="setting">
        ...
        <p
          class="description"
        >Nom de l'arène, tel que vous souhaitez le voir apparaître sur Discord et le map</p>
    ...
    <div class="settings-section">  
      ...
      <div v-if="gym" class="setting">
        ...
        <p class="description">
          Les alias de nom sont utilisés lors de la détection des raids pour identifier une arène dont le nom serait mal lu depuis l'image.
          <a href="https://www.pokematos.fr/documentation/alias-de-pois/">En savoir plus</a>
        </p>
        <div class="alias" v-for="(alias, index) in aliases">
          <input v-model="alias.name" type="text" />
          <v-btn small flat fab @click="removeAlias(index)">
            <v-icon>delete</v-icon>
          </v-btn>
        </div>
        <div class="alias__add">
          <v-btn small fab @click="addAlias">
            <v-icon>add</v-icon>
          </v-btn>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    getId() {
      let routeId =
        this.$route.params.poi_id &&
        Number.isInteger(parseInt(this.$route.params.poi_id))
          ? parseInt(this.$route.params.poi_id)
          : false;
  ...
  methods: {
    ...
    create(args) {
      ...
      axios
        .post(
          "/api/user/cities/" + this.$store.state.currentCity.id + "/gyms",
          args
        )
        .then(res => {
          this.$store.dispatch("fetchGyms");
